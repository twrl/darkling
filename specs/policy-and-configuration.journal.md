...
- [Constrained agent](./constrained-agent.spec.md) — owns budget and flush policy parameters (formerly the event system spec, now consolidated into the constrained agent). The new spec's contract is consistent with the constrained agent's "Policy parameters" section; the parameter table and the requirement that all parameters have defined values are preserved. The constrained agent spec links to the new spec for the cross-cutting contract. No contradiction.
...
- [Constrained agent (overspend/carryover)](./constrained-agent.spec.md#budget) — the constrained agent spec (formerly deferring overspend and carryover to "the event system spec or a dedicated policy spec") now owns both the mechanism and the policy. The new spec satisfies the "dedicated policy spec" possibility by providing the cross-cutting contract, while the specific overspend/carryover parameters remain in the constrained agent spec. Consistent.
- [Agent safety](./agent-safety.spec.md) — references the cost of `safety_consult` as a policy parameter owned by the constrained agent spec (formerly the event system). The new spec does not change this ownership. Consistent.
...
- **Domain spec linkage.** The new spec is established, and the domain specs ([Constrained agent](./constrained-agent.spec.md), [Content-first retrieval](./content-first-retrieval.spec.md)) now link to it. The constrained agent spec's "Policy parameters" section links to the new spec for the cross-cutting contract (following the consolidation of the former event system spec into the constrained agent spec).
