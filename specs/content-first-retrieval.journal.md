...
## Client-side caching mechanism moved here from Usage and deployment

After the usage-and-deployment spec was refocused on assembly and deployment, the client-side caching mechanism was moved into this specification's new [Client-side caching](#client-side-caching) section. This resolves the "cache invalidation interface" gap noted above: the `since` parameter and the invalidation-list result shape are now defined here.

The new section owns: the cache (client-side optimisation, backend is source of truth), prefetch, cache invalidation (the `since` parameter + invalidation list, piggybacked on retrieval calls), the table of contents (fetched at bootstrap, invalidated via the same list), offline (not offline-first), and the bundled fallback. The deployment-level topology (where retrieval executes, the persistent-store dependency) remains in usage-and-deployment; this spec owns the retrieval interface and the caching behaviour. The Storage and indexing section's deployment reference was updated to point to the new section.
