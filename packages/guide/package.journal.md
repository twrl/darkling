...
This journal records the development of the `@darkling/guide` package — the implementation of [constrained-agent.spec.md](../../specs/constrained-agent.spec.md) (which now includes the former event-system specification — events, queue, flush, triggering, and budget), the `safety_consult` tool of [agent-safety.spec.md](../../specs/agent-safety.spec.md), and the LLM provider abstraction of [usage-and-deployment.spec.md](../../specs/usage-and-deployment.spec.md) as defined by [package.spec.md](./package.spec.md). It is non-normative; the specifications take precedence.

## `BudgetPolicy.premium` as a map; `random` defaulting in the worker

Established through dialogue with the user while wiring the Guide's frontend services. The root spec ([Constrained agent — Budget composition](../../specs/constrained-agent.spec.md#budget-composition), formerly the event system spec) defines the premium as "a function of the event types in the queue" and lists the "budget premium function" as a policy parameter; the guide package spec described `BudgetPolicy.premium` as a `(eventTypes: string[]) => number` function. The user directed two changes:
...
- **One package, the constrained-agent area.** The package is named `@darkling/guide` (after the actor) rather than `@darkling/constrained-agent` (after the spec), matching the repo's idiom. It conforms to `constrained-agent.spec.md` (which now includes the former event-system budget policy) and secondarily to the `safety_consult` of `agent-safety.spec.md`, and the LLM provider abstraction of `usage-and-deployment.spec.md`.
...
[Constrained agent](../../specs/constrained-agent.spec.md#interaction-triggering) owns the event queue, the per-event roll, and the trigger. [Constrained agent](../../specs/constrained-agent.spec.md#agentic-model) defines that when an interaction is triggered, the current event queue is sent to the model. The `AgentLoop.runInteraction` therefore takes the flushed event queue as an argument and runs one interaction; it does not own the queue, the roll, or the trigger. The host (the frontend) is responsible for queueing events, rolling for flush, and calling `runInteraction` when a flush occurs. This keeps the boundary between the flush policy (trigger) and the interaction model clean, as anticipated by the constrained-agent journal.
...
The budget implementation was rewritten to conform to the revised [Constrained agent](../../specs/constrained-agent.spec.md#budget) budget policy (the spec change is recorded in `specs/constrained-agent.journal.md` under the revision and consolidation sections; formerly `specs/event-system.journal.md`). The earlier `maxOverspend` / `maxCarryover` / `minCarryover` / `carryoverDecay` / `carryoverDecayThreshold` policy parameters and the deterministic `canDispatch(cost)` bound are gone, replaced by:
...
- **Prompt construction.** The system message is a simple rendering of the status object. A real Guide prompt (personality, annotations, safety encouragement) is the backend's concern, composed from the Guide definition (established by [Usage and deployment](../../specs/usage-and-deployment.spec.md#guide-definition)) and annotations (defined by [Annotations](../../specs/annotations.spec.md)), around the constrained-agent framing this package's provider supplies. The provider provides the constrained-agent framing; the backend composes the full prompt around it.
